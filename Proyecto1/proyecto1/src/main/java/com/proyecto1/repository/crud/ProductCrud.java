...
public interface ProductCrud extends CrudRepository<Product, Integer> {
    ...
    @Query(value = "SELECT * FROM reporte_productos_mas_vendidos ORDER BY sum desc LIMIT 10;",nativeQuery = true)
    ArrayList<String> mostSelledProducts();

    /*Top productos con mas ingresos*/
    @Query(value = "SELECT * FROM reporte_productos_mas_ingresos ORDER BY sum DESC LIMIT 10;",nativeQuery = true)
    ArrayList<String> productsWithMostIncome();

    /*Top 5 productos mas vendidos por sucursal*/
    @Query(value = "SELECT * FROM vista_productos_mas_vendidos_sucursal WHERE id_sucursal = ? ORDER BY sum DESC LIMIT 5;",nativeQuery = true)
    ArrayList<String> mostSelledProductsByBranch(int idSucursal);

    /*Top 5 productos mas ingresos por sucursal*/
    @Query(value = "SELECT * FROM reporte_productos_mas_ingresos_sucursal WHERE id_sucursal = ? ORDER BY sum DESC LIMIT 5;", nativeQuery = true)
