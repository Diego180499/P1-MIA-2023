...
//*****************************************************************************************
//llenado de tabla
let contenidoTabla = "<tr><th>Sucursal</th><th>Cantidad Ventas</th></tr>";
sucursalesMasVentas();


/* CONSULTAS A LA API REST */
function sucursalesMasVentas(){
    const request = new Request("http://localhost:8080/reports/branchWithMostSales");
    fetch(request)
        .then(res => res.json())
        .then(response => {
...
function llenarTabla(data){
    for(let i=0; i< data.length; i++){
        contenidoTabla += "<tr><td>"+data[i].branchName+"</td><td>"+data[i].salesAmount+"</td></tr>";
    }
