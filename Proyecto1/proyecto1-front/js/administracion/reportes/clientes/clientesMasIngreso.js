...
//*****************************************************************************************
//llenado de tabla
let contenidoTabla = "<tr><th>Nit</th><th>Nombre</th><th>Apellido</th><th>Ingreso</th></tr>";
productosMasVendidos();


/* CONSULTAS A LA API REST */
function productosMasVendidos(){
    const request = new Request("http://localhost:8080/reports/clientWithMostIncome");
    fetch(request)
        .then(res => res.json())
        .then(response => {
...
function llenarTabla(data){
    for(let i=0; i< data.length; i++){
        contenidoTabla += "<tr><td>"+data[i].nit+"</td><td>"+data[i].name+"</td><td>"+data[i].lastName+"</td><td>"+data[i].income+"</td></tr>";
    }
