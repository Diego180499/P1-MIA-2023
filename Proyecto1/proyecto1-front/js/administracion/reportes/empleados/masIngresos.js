...
//*****************************************************************************************
//llenado de tabla
let contenidoTabla = "<tr><th>DPI</th><th>Nombre</th><th>Apellido</th><th>Ingresos</th></tr>";
clientesMasVentas();


function clientesMasVentas(){
    const request = new Request("http://localhost:8080/reports/employeeWithMostIncome");
    fetch(request)
        .then(res => res.json())
        .then(response => {
...
function llenarTabla(data){
    for(let i=0; i<data.length; i++){
        contenidoTabla += "<tr><td>"+data[i].dpi+"</td><td>"+data[i].name+"</td><td>"+data[i].lastName+"</td><td>"+data[i].income+"</td></tr>";
    }
