...
//*****************************************************************************************
//llenado de tabla
let contenidoTabla = "<tr><th>Nombre Producto</th><th>Cantidad Ventas</th></tr>";
productosMasVendidos();


/* CONSULTAS A LA API REST */
function productosMasVendidos(){
    const request = new Request("http://localhost:8080/reports/mostSelledProduct");
    fetch(request)
        .then(res => res.json())
        .then(response => {
...
function llenarTabla(data){
    for(let i=0; i< data.length; i++){
        contenidoTabla += "<tr><td>"+data[i].productName+"</td><td>"+data[i].saleAmount+"</td></tr>";
    }
